...
### Sun Sep 13, afternoon: people (agent side, no shell change)
- `agent/src/tempo/people.py` + `faces.py` + `voices.py` + `siyi.py`, `tempo people --frames DIR`. Faces on the exported frames (YuNet + ArcFace w600k_mbf, lifted from FaceLock), unprojected through the LiDAR map into the scene; one `note` bubble per visible head, kept 0.30 m to the wearer's right of the face via `pose`, text via `note-update`, closed 3 s after the face leaves. Open mic: energy VAD → mlx-whisper → ECAPA voiceprint (Amelia's sidecar model). "I'm Alice" names the speaker's face and voice; "this is Bob" names the largest face in view. Utterances are filed per person; `people.describe()` feeds `perception.scene_text()` so `tempo ask who is this` works. Store `~/.config/tempo/people.json`. Optional siyi (CRM) notes via `TEMPO_SIYI_URL`/`TEMPO_SIYI_KEY`.
